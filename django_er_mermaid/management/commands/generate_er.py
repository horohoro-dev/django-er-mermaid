...
# Apps to exclude from scanning (Django built-in + common third-party)
EXCLUDED_APPS = {
    "admin",
    "auth",
    "contenttypes",
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            required=False,
            default=None,
            help='Apps to include (comma-separated). If not specified, all project apps are scanned.',
        )
        parser.add_argument(
            "-o",
            "--output",
    ...
    def _get_all_project_apps(self) -> list[str]:
        """Get all project apps, excluding Django built-ins and this package."""
        result = []
        for config in apps.get_app_configs():
            label = config.label
            if label in EXCLUDED_APPS:
                continue
            # Exclude this package itself
            if label == "django_er_mermaid":
                continue
            result.append(label)
        return result

    def _parse_apps(self, apps_arg: str | None) -> list[str]:
        if not apps_arg:
            return self._get_all_project_apps()
        return [i.strip() for i in apps_arg.split(",")]
